...
plot(plt.t(1:end), plt.armL.a_ma, plt.t(1:end), plt.armL.a_tool(1,:), plt.t(1:end), plt.a_rc(1,:), plt.t(1:end), plt.armL.a_jl(1,:), "LineWidth",2);
grid on;
xlabel("Time [s]");
ylabel("Activation");
%fontsize(gca,22,"points");
...
%lgd = legend("MA","T","RC","Location","northeastoutside");
lgd = legend("MA","T","RC", "JL");
%fontsize(lgd,22,"points");
ylim([-0.1 1.1]);

% Right arm
fig = figure("Name","Right Arm Activation Function");
plot(plt.t(1:end), plt.armR.a_ma, plt.t(1:end), plt.armR.a_tool(1,:), plt.t(1:end), plt.a_rc(1,:), plt.t(1:end), plt.armR.a_jl(1,:), "LineWidth",2);
grid on;
xlabel("Time [s]");
ylabel("Activation");
%fontsize(gca,22,"points");
